...
export async function generateInvoiceForStudent(opts: {
  ...
  // Plan 04: throughDate cutoff restricts to on-or-before a specified date
  // (manual generate only; cadence always bills all unbilled by passing no cutoff).
  const unbilledSessions = await db
    ...
    .where(
      and(
        ...
        opts.throughDate ? lte(sessions.date, opts.throughDate) : undefined,
      ),
    )
